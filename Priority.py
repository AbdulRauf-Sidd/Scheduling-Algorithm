...
for x in range(no):
	...
	pri = eval(input("Enter Priority for process " + str(x) + ": "));
	p = process(x, at, bt, pri);
	total.append(p);
...
#print ct
print("ID \tAT \tBT \tCT \tTAT \tWT \tPri");
totaltat = 0
totalwt = 0
for i in range(len(total)):
	print(str(total[i].id) + " \t" + str(total[i].at) + " \t" + str(total[i].bt) + " \t" + str(total[i].ct) +
	" \t" + str(total[i].tat) + " \t" + str(total[i].wt) + " \t" + str(total[i].pri));
